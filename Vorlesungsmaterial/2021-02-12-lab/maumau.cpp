...
int main() {
  ...
  string spieler1 = "Niemand";
  string spieler2 = "Keiner";

  vector<Spielkarte> kartenstapel = skatBlatt();
  vector<Spielkarte> ablagestapel;
  ziehen(kartenstapel, ablagestapel);
  vector<Spielkarte> hand1;
  vector<Spielkarte> hand2;

  mischen(kartenstapel);
  geben(kartenstapel, hand1);
  geben(kartenstapel, hand2);

  int spieler = 1;
  while (hand1.size() != 0 && hand2.size() != 0)
  {
    string name = spieler == 1 ? spieler1 : spieler2;
    vector<Spielkarte> & hand = spieler == 1 ? hand1 : hand2;
