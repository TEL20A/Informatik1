...
bool contains(T begin, T end, int x)
{
  ...
  return (*begin == x) || contains(begin+1, end, x);
}
...
{
  if (begin == end) { return 0; }
  return ((*begin == x) ? 1 : 0) + count(begin+1, end, x);
}

template<typename T>
void print(T begin, T end)
{
  if (begin == end) { cout << endl; return; }
  cout << *begin << " ";
  print(begin+1, end);
}
...
{
  if (begin == end) { return {}; }
  auto result = reverse(begin+1, end);
...
{
  if (begin == end) { return {}; }
  auto result = filterk(begin, end-1, p);
  if (*(end-1) < p)
  {
    result.push_back(*(end-1));
  }
